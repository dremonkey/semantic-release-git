...
async function getModifiedFiles(execaOptions) {
  return execa
    .sync('git', ['ls-files', '-m', '-o'], execaOptions)
    .stdout.split('\n')
    .map((file) => file.trim())
...
async function add(files, execaOptions) {
  const shell = execa.sync('git', ['add', '--force', '--ignore-errors', ...files], {...execaOptions, reject: false});
...
async function commit(message, execaOptions) {
  execa.sync('git', ['commit', '-m', message], execaOptions);
}
 ...
 */
function push(origin, branch, execaOptions) {
  console.log(origin, branch, execaOptions);
  const response = execa.sync('git', ['push', '--tags', origin, `HEAD:${branch}`], execaOptions);
  console.log(response);
  const response2 = execa.sync('git', ['log'], execaOptions);
...
async function gitHead(execaOptions) {
  return execa.sync('git', ['rev-parse', 'HEAD'], execaOptions).stdout;
}
